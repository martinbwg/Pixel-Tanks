...
const connectionString =
    "mongodb+srv://cs641311:355608-G38@cluster0.z6wsn.mongodb.net/?retryWrites=true&w=majority",
  port = 80;

const app = express();
const client = new MongoClient(connectionString, { poolSize: 10 });
const filter = new Filter(),
  tokgen = new TokenGenerator(256, TokenGenerator.BASE62),
  tokens = new Set(),
  sockets = [];
let db;
...
const routes = {
  auth: async ({ username, type, password }, socket) => {
    if (!username.trim() || /\s|:/.test(username))
      return socket.send({ status: "error", message: "Invalid username." });
    if (username !== filter.clean(username))
      return socket.send({
        status: "error",
        message: "Username contains inappropriate word.",
      });
    const item = await db.findOne({ username }),
    ...
    if (type === "signup") {
      ...
      if (item.password !== password)
        return socket.send({ status: "error", message: "Incorrect password." });
    } else return;
  ...
  database: async ({ token, username, type, key, value }, socket) => {
    ...
    if (!valid(token, username))
      return socket.send({ status: "error", message: "Invalid token." });
    try {
      if (type === "get") {
        socket.send({
          status: "success",
          type: "get",
          data: await db.findOne({ username }, { projection: { password: 0 } }),
        });
      } else if (type === "set") {
...
app.ws("/", (socket) => {
  ...
  socket.on("message", (data) => {
    ...
    try {
      ...
      return socket.send({
        status: "error",
        message: "Invalid or no operation.",
      });
    }
  });
});
...
app.get("/*", async (req, res) =>
  res
    ...
    .end(await fs.readFile("./public/js/pixel-tanks.js")),
);
