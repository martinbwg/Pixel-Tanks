...
class A {
  ...
  static assign(obj, keys, values) {
    A.each(keys, function(d) {
      obj[this] = d.values[d.i]
    }, null, {
      values: values
    });
  }
  ...
  onclick() {
    for (const b of this.buttons) {
      if (A.collider({
          ...
          x: this.render[0] + b[0],
          y: this.render[1] + b[1],
          w: b[2] * this.render[2] / 1600,
          h: b[3] * this.render[3] / 1000
        })) {
...
class PixelTanks {
  ...
  static boot() {
    ...
    Menus.menus = {
      start: {
        buttons: [
          [544, 648, 216, 116, function() {
            PixelTanks.auth(this.username, this.password, 'login')
          }, true],
          [840, 648, 216, 116, function() {
            PixelTanks.auth(this.username, this.password, 'signup')
          }, true],
          [564, 392, 456, 80, function() {
            this.type = 'username'
          }, false],
          [564, 520, 456, 80, function() {
            this.type = 'password'
          }, false],
        ],
        listeners: {
      ...
      multiplayer: {
        buttons: [
          [424, 28, 108, 108, 'main'],
          [340, 376, 416, 116, function() {
            this.gamemode = 'ffa'
          }, true],
          [340, 532, 416, 116, function() {
            this.gamemode = 'duels'
          }, true],
          [340, 688, 416, 116, function() {
            this.gamemode = 'tdm'
          }, true],
          [340, 844, 416, 116, function() {
            this.gamemode = 'juggernaut'
          }, true],
        ...
        listeners: {
          keydown: function(e) {
            ...
            this.socket = new MegaSocket('ws://' + this.ip, {
              keepAlive: false,
              reconnect: true,
              autoconnect: true
            });
      ...
      crate: {
        buttons: [
          ...
          [232, 308, 488, 488, function() {
            PixelTanks.openCrate(0)
          }, false],
          [880, 308, 488, 488, function() {
            PixelTanks.openCrate(1)
          }, false],
        ],
        listeners: {},
        cdraw: function() {
      ...
      inventory: {
        buttons: [
          ...
          [1112, 816, 88, 88, function() {
            PixelTanks.switchTab('classTab')
          }, true],
          [400, 816, 88, 88, function() {
            PixelTanks.switchTab('itemTab', 1)
          }, true],
          [488, 816, 88, 88, function() {
            PixelTanks.switchTab('itemTab', 2)
          }, true],
          [576, 816, 88, 88, function() {
            PixelTanks.switchTab('itemTab', 3)
          }, true],
          [664, 816, 88, 88, function() {
            PixelTanks.switchTab('itemTab', 4)
          }, true],
          [756, 220, 88, 88, function() {
            PixelTanks.switchTab('cosmeticTab')
          }, true],
          [532, 220, 88, 88, function() {
            PixelTanks.switchTab('deathEffectsTab')
          }, true],
        ],
        listeners: {
          mousedown: function(e) {
      ...
      shop: {
        buttons: [
          ...
          [880, 208, 488, 96, function() {
            /* ded tab */ }, true],
          [496, 404, 176, 176, function() {
            PixelTanks.purchase(0)
          }, true],
          [712, 404, 176, 176, function() {
            PixelTanks.purchase(1)
          }, true],
          [928, 404, 176, 176, function() {
            PixelTanks.purchase(4)
          }, true],
          [496, 620, 176, 176, function() {
            PixelTanks.purchase(2)
          }, true],
          [712, 620, 176, 176, function() {
            PixelTanks.purchase(5)
          }, true],
          [928, 620, 176, 176, function() {
            PixelTanks.purchase(3)
          }, true],
        ],
        listeners: {},
        cdraw: function() {
  ...
  static openCrate(type) {
    try {
      const price = type ? 5 : 1,
        name = type ? 'deathEffects' : 'cosmetics',
      ...
      let number = Math.floor(Math.random() * (crate[type][rarity].length)),
        item;
      for (const e in this.images[name])
        if (e === crate[type][rarity][number]) item = this.images[name][e];
...
class Tank {
  constructor(ip, multiplayer, gamemode) {
    ...
    if (multiplayer) {
      this.socket = new MegaSocket(`ws://${ip}`, {
        keepAlive: false,
        reconnect: false,
        autoconnect: true
      });
        ...
        switch (data.event) {
          ...
          case 'gameover':
            ...
            if (data.type === 'victory') {
              Menus.menus.victory.stats = {
                kills: 'n/a',
                coins: 'n/a'
              };
              Menus.trigger('victory');
            } else {
              Menus.menus.defeat.stats = {
                kills: 'n/a',
                coins: 'n/a'
              };
          ...
          case 'override':
            ...
            break;
          case 'kill':
            const crates = Math.floor(Math.random() * 2) + 1,
  ...
  drawTank(t) {
    ...
    if (t.dedEffect) {
      const {
        ...
        frames,
        kill
      } = PixelTanks.images.deathEffects[t.dedEffect.id + '_'];
  ...
  frame() {
    ...
    this.fps++;
    const t = this.hostupdate.pt,
      b = this.hostupdate.b,
      s = this.hostupdate.s,
      a = this.hostupdate.ai,
      e = this.hostupdate.d;
    if (this.dx) {
    ...
    var l = 0,
      len = Math.min((this.showChat || this.hostupdate.logs.length < 3) ? this.hostupdate.logs.length : 3, 30);
    while (l < len) {
      GUI.draw.fillStyle = '#000000';
      GUI.draw.globalAlpha = .2;
  ...
  chat(e) {
    ...
    if (e.keyCode === 13) {
      if (this.msg !== '') {
        this.socket.send(this.msg.charAt(0) === '/' ? {
          type: 'command',
          data: this.msg.replace('/', '').split(' ')
        } : {
          type: 'chat',
          msg: this.msg
        });
  ...
  keyup(e) {
    ...
    if ([87, 65, 68, 83].includes(e.keyCode)) {
      this.b = false;
      if (this.key[65]) this.keyStart({
        keyCode: 65
      });
      if (this.key[68]) this.keyStart({
        keyCode: 68
      });
      if (this.key[87]) this.keyStart({
        keyCode: 87
      });
      if (this.key[83]) this.keyStart({
        keyCode: 83
      });
    }
  }
  mousemove(e) {
    this.mouse = {
      x: (e.clientX - (window.innerWidth - window.innerHeight * 1.6) / 2) / PixelTanks.resizer,
      y: e.clientY / PixelTanks.resizer
    };
  ...
  useItem(id, slot) {
    ...
    if (id === 'duck_tape') {
      ...
      this.tank.airstrike = {
        x: this.mouse.x + this.tank.x - 800,
        y: this.mouse.y + this.tank.y - 500
      };
      cooldown = 20000;
    }
    this.timers.items[slot] = {
      cooldown: cooldown,
      time: Date.now()
    };
  ...
  keyStart(e) {
    ...
    if (k === 9) {
      ...
      if (c === 'stealth' && !this.halfSpeed) {
        ...
        for (let i = -30; i < 30; i += 5) this.tank.fire.push({
          ...toPoint(this.tank.r + i),
          type: 'fire',
          r: this.tank.r + i
        });
        this.timers.class = {
  ...
  constructor(level) {
    const levels = [
      [
        ...
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B4", "B4", "B4", "B4", "B4", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B0", "B0", "B0", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B4", "B0", "B0", "B0", "B0", "B0", "B4", "B0", "B4", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B4", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B4", "B0", "B0", "B0", "B0", "B0", "B4", "B0", "B0", "B0", "B0", "B4", "B1", "B4", "B0", "B1", "B0", "B1", "B1", "B4", "B0", "B0", "B4", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B4", "B0", "B0", "A1", "B0", "B0", "B1", "B0", "B4", "B0", "B0", "B1", "A1", "B1", "B0", "B0", "B0", "B0", "A0", "B4", "B0", "B0", "B4", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B4", "B0", "B0", "B0", "B0", "B0", "B4", "B0", "B0", "B0", "B0", "B4", "B1", "B4", "B0", "B1", "B0", "B1", "B1", "B4", "B0", "B0", "B4", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B4", "B0", "B0", "B0", "B0", "B0", "B4", "B0", "B4", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B4", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B0", "B0", "B0", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B0", "B0", "B4", "B5", "B5", "B1", "B5", "B5"],
        ["B5", "B5", "B4", "B0", "B0", "B0", "B0", "B0", "B4", "B4", "B4", "B4", "B4", "B1", "B0", "B0", "B1", "B0", "B0", "B2", "B3", "B2", "B0", "B0", "B4", "B5", "B1", "B1", "B1", "B5"],
        ["B5", "B5", "B4", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B1", "B0", "B2", "B0", "B0", "B0", "B0", "B0", "B0", "B4", "B1", "B5", "B1", "B5", "B1"],
        ["B5", "B5", "B4", "B0", "B0", "S", "B0", "B0", "B0", "B0", "B0", "B1", "B0", "B0", "B0", "B0", "B1", "B0", "B0", "B2", "B0", "B2", "B0", "B0", "B4", "B5", "B5", "B1", "B5", "B5"],
        ["B5", "B5", "B4", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B1", "B0", "B1", "B0", "B0", "B2", "B3", "B0", "B0", "B4", "B5", "B5", "B1", "B5", "B5"],
        ["B5", "B5", "B4", "B0", "B0", "B0", "B0", "B0", "B4", "B4", "B4", "B4", "B4", "B0", "B1", "B1", "B0", "B0", "B2", "B0", "B0", "B0", "B0", "B0", "B4", "B5", "B5", "B1", "B5", "B5"],
        ["B5", "B5", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B5", "B5", "B5", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B4", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5"],
      ...
      [
        ...
        ["B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B0", "B0", "B0", "B0", "B0", "B0", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B4", "B4", "B4", "B4", "B4", "B5", "B5", "B5", "B4", "B4", "B4", "B4", "B4", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B4", "B0", "B0", "B0", "B4", "B4", "B5", "B4", "B4", "B0", "B0", "B0", "B4", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B5", "B0", "B0", "B0", "B0", "B0", "B0", "B0"],
        ["B5", "B4", "B0", "A1", "B0", "B0", "B4", "B4", "B4", "B0", "B0",