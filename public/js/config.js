let images = {
  ...
  emotes: {
    // type: 0=loop 1=play once 2=static
...
let menus = {
  ...
  main: {
    ...
    cdraw: function () {
      ...
      if (
        PixelTanks.userData.cosmetic !== "" &&
        PixelTanks.userData.cosmetic !== undefined
      )
        GUI.drawImage(
          PixelTanks.images.cosmetics[PixelTanks.userData.cosmetic],
          1200,
          600,
          160,
          180,
      ...
      mousedown: function (e) {
        ...
        for (const c of levelCoords) {
          if (
            ...
            PixelTanks.user.player = new Tank(
              levelCoords.indexOf(c) + 3,
              false,
              null,
            );
          }
  ...
  multiplayer: {
    ...
    listeners: {
      keydown: function (e) {
        ...
        this.socket = new MegaSocket(
          (window.location.protocol === "https:" ? "wss://" : "ws://") +
            this.ip,
          { keepAlive: false, reconnect: true, autoconnect: true },
        );
  ...
  crate: {
    ...
    cdraw: function () {
      GUI.drawText(
        "Crates: " + PixelTanks.userData.stats[1],
        800,
        260,
        30,
        "#ffffff",
  ...
  keybinds: {
    buttons: [[40, 40, 120, 120, "main", true]],
    listeners: {},
    cdraw: function () {},
  },
  help: {
  ...
  helpinventory: {
    buttons: [[120, 132, 120, 120, "help", true]],
    listeners: {},
    cdraw: function () {},
  },
  helpcosmetic: {
  ...
  inventory: {
    ...
    listeners: {
      mousedown: function (e) {
        const { x, y } = Menus;
        if (this.classTab) {
          if (x < 688 || x > 912 || y < 334 || y > 666)
            return (this.classTab = false);
          ...
          if (x < 580 || x > 1020 || y < 334 || y > 666)
            return (this.itemTab = false);
          const key = {
            airstrike: [600, 354],
            super_glu: [708, 354],
            duck_tape: [816, 354],
          ...
          for (const item in key) {
            if (collision(x, y, 0, 0, key[item][0], key[item][1], 80, 80)) {
              if (!PixelTanks.userData.items.includes(item)) {
                ...
                alert(
                  "You are not allowed to have more than 1 of the same item",
                );
              return;
            }
          }
        } else if (this.cosmeticTab) {
          if (x < 518 || x > 1082 || y < 280 || y > 720)
            return (Menus.menus.inventory.cosmeticTab = false);
          for (let i = 0; i < 16; i++) {
            if (
              collision(
                x,
              ...
              if (e.button === 0) {
                PixelTanks.userData.cosmetic =
                  PixelTanks.userData.cosmetics[this.cosmeticMenu * 16 + i];
              } else {
                PixelTanks.userData.cosmetics.splice(
                  this.cosmeticMenu * 16 + i,
                  1,
                );
              }
              return;
          ...
          if (x < 518 || x > 1082 || y < 280 || y > 720)
            return (Menus.menus.inventory.deathEffectsTab = false);
          for (let i = 0; i < 16; i++) {
            if (
              collision(
                x,
              ...
              if (e.button === 0) {
                PixelTanks.userData.deathEffect =
                  PixelTanks.userData.deathEffects[
                    this.deathEffectsMenu * 16 + i
                  ];
              } else {
                PixelTanks.userData.deathEffects.splice(
                  this.deathEffectsMenu * 16 + i,
                  1,
                );
              }
      ...
      keydown: function (e) {
        ...
        if (this.cosmeticTab) {
          ...
          if (
            e.keyCode === 39 &&
            this.cosmeticMenu + 1 !==
              Math.ceil(PixelTanks.userData.cosmetics.length / 16)
          )
            this.cosmeticMenu++;
        }
      },
    ...
    cdraw: function () {
      ...
      GUI.drawText(PixelTanks.user.username, 300, 420, 80, "#000000", 0.5);
      GUI.drawText("Coins: " + coins, 300, 500, 50, "#FFE900", 0.5);
      GUI.drawText("Rank: " + rank, 300, 550, 50, "#FF2400", 0.5);
      GUI.drawText("Level Up Progress", 1400, 400, 50, "#000000", 0.5);
      GUI.drawText(
        (rank < 20 ? coins + "/" + coinsUP : "MAXED") + " Coins",
        1400,
        500,
        50,
        rank < 20 ? (coins < coinsUP ? "#FF2400" : "#90EE90") : "#63666A",
        0.5,
      );
      GUI.drawText(
        (rank < 20 ? xp + "/" + xpUP : "MAXED") + " XP",
        1400,
        550,
        50,
        rank < 20 ? (xp < xpUP ? "#FF2400" : "#90EE90") : "#63666A",
        0.5,
      );
      if (coins < coinsUP || xp < xpUP || rank > 19) {
        GUI.draw.fillStyle = "#000000";
        GUI.draw.globalAlpha = 0.7;
        GUI.draw.fillRect(1064, 458, 88, 88);
        GUI.draw.globalAlpha = 1;
      }
      for (let i = 0; i < 4; i++)
        GUI.drawImage(
          PixelTanks.images.items[PixelTanks.userData.items[i]],
          [404, 492, 580, 668][i],
          820,
          80,
          80,
      ...
      PixelTanks.renderTop(
        680,
        380,
        240,
        PixelTanks.userData.color,
        ((-Math.atan2(this.target.x, this.target.y) * 180) / Math.PI + 360) %
          360,
      );
      GUI.drawImage(
        PixelTanks.images.tanks.top,
        680,
        380,
        240,
        270,
      ...
      if (PixelTanks.userData.class)
        GUI.drawImage(
          PixelTanks.images.menus.classTab,
          1112,
          816,
          88,
          88,
          ...
          0,
          undefined,
          key[PixelTanks.userData.class][0],
          key[PixelTanks.userData.class][1],
          44,
          44,
        );
      if (PixelTanks.userData.cosmetic)
      ...
      const deathEffectData =
        PixelTanks.images.deathEffects[PixelTanks.userData.deathEffect + "_"];
      if (PixelTanks.userData.deathEffect)
        GUI.drawImage(
          PixelTanks.images.deathEffects[PixelTanks.userData.deathEffect],
          536,
          224,
          80,
          80,
          ...
          0,
          undefined,
          (Math.floor((Date.now() - this.time) / deathEffectData.speed) %
            deathEffectData.frames) *
            200,
          0,
          200,
          200,
      ...
      if (
        ...
        this.itemTab ||
        this.cosmeticTab ||
        this.deathEffectsTab
      ) {
        Menus.menus.inventory.buttonEffect = false;
        GUI.drawImage(PixelTanks.images.blocks.void, 0, 0, 1600, 1600, 0.7);
      }
      if (this.classTab) {
        ...
        if (PixelTanks.userData.class === "tactical")
          GUI.draw.strokeRect(701, 348, 88, 88);
        else if (PixelTanks.userData.class === "fire")
          GUI.draw.strokeRect(701, 456, 88, 88);
        else if (PixelTanks.userData.class === "medic")
          GUI.draw.strokeRect(701, 565, 88, 88);
        else if (PixelTanks.userData.class === "stealth")
          GUI.draw.strokeRect(814, 348, 88, 88);
        else if (PixelTanks.userData.class === "builder")
          GUI.draw.strokeRect(814, 456, 88, 88);
        else if (PixelTanks.userData.class === "warrior")
          GUI.draw.strokeRect(814, 565, 88, 88);
      } else if (this.itemTab) {
        ...
        const a = this.cosmeticMenu === 0,
          b =
            this.cosmeticMenu ===
            Math.floor(PixelTanks.userData.cosmetics.length / 16);
        GUI.drawImage(
          PixelTanks.images.menus.cosmeticTab,
          518 + (a ? 62 : 0),
          280,
          564 - (a ? 62 : 0) - (b ? 62 : 0),
          440,
        ...
        for (
          let i = this.cosmeticMenu * 16;
          i <
          Math.min(
            (this.cosmeticMenu + 1) * 16,
            PixelTanks.userData.cosmetics.length,
          );
          i++
        ) {
          GUI.drawImage(
            PixelTanks.images.cosmetics[PixelTanks.userData.cosmetics[i]],
            598 + (i % 4) * 108,
            298 + Math.floor((i % 16) / 4) * 108,
            88,
            88,
            1,
          );
          if (
            PixelTanks.userData.cosmetics[i] === PixelTanks.userData.cosmetic
          ) {
        ...
        const a = this.deathEffectsMenu === 0,
          b =
            this.deathEffectsMenu ===
            Math.floor(PixelTanks.userData.deathEffects.length / 16);
        GUI.drawImage(
          PixelTanks.images.menus.deathEffectsTab,
          518 + (a ? 62 : 0),
          280,
          564 - (a ? 62 : 0) - (b ? 62 : 0),
          440,
        ...
        for (
          let i = this.deathEffectsMenu * 16;
          i <
          Math.min(
            (this.deathEffectsMenu + 1) * 16,
            PixelTanks.userData.deathEffects.length,
          );
          i++
        ) {
          const d =
            PixelTanks.images.deathEffects[
              PixelTanks.userData.deathEffects[i] + "_"
            ];
          GUI.drawImage(
            PixelTanks.images.deathEffects[PixelTanks.userData.deathEffects[i]],
            598 + (i % 4) * 108,
            298 + Math.floor((i % 16) / 4) * 108,
            88,
            88,
            ...
            0,
            undefined,
            (Math.floor((Date.now() - this.time) / d.speed) % d.frames) * 200,
            0,
            200,
            200,
          );
          if (
            PixelTanks.userData.deathEffects[i] ===
            PixelTanks.userData.deathEffect
          ) {
  ...
  shop: {
    ...
    cdraw: function () {
      GUI.drawText(
        PixelTanks.userData.stats[0] + " coinage",
        800,
        350,
        50,
        0x000000,
